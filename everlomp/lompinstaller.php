...
if (!isset($_SESSION['everlomp_wp_migration_csrf']) || !is_string($_SESSION['everlomp_wp_migration_csrf']) || strlen($_SESSION['everlomp_wp_migration_csrf']) < 32) {
    $_SESSION['everlomp_wp_migration_csrf'] = bin2hex(random_bytes(32));
}
$wpMigrationCsrf = (string) $_SESSION['everlomp_wp_migration_csrf'];

if (!isset($_SESSION['everlomp_wp_migration_token']) || !is_string($_SESSION['everlomp_wp_migration_token']) || preg_match('/^[a-f0-9]{32}$/', $_SESSION['everlomp_wp_migration_token']) !== 1) {
    $_SESSION['everlomp_wp_migration_token'] = bin2hex(random_bytes(16));
}
$wpMigrationToken = (string) $_SESSION['everlomp_wp_migration_token'];
$wpMigrationStageBase = rtrim(sys_get_temp_dir(), '/') . '/everlomp-wordpress-migration';
...
function runDirectHelper(string $helper, string $action, string $stdin = ''): array
{
...
[$contractLibraryCode, $contractLibraryStdout] = runEverlompHelper(
...
if ($contractLibraryCode === 0 && $contractLibraryStdout !== '') {
    ...
    if (is_array($contractLibraryDecoded)) {
        ...
        if ($libraryDir !== '') {
            $hotpocketContractLibraryDir = $libraryDir;
        }
...
if (
    ...
    && (string) ($_GET['download'] ?? '') === 'hotpocket-contract'
) {
    $name = trim((string) ($_GET['name'] ?? ''));

    // Only allow normal ZIP filenames.
    if (
        $name === ''
        || preg_match('/^[A-Za-z0-9._-]+\.zip$/i', $name) !== 1
    ) {
        http_response_code(400);
        header('Content-Type: text/plain; charset=utf-8');
        echo "Invalid smart contract ZIP.\n";
        exit;
    }

    // Only files that actually appeared in the contract library may be downloaded.
    $allowedNames = array_values(array_map(
    ...
    if (!in_array($name, $allowedNames, true)) {
        http_response_code(404);
        header('Content-Type: text/plain; charset=utf-8');
        echo "Smart contract ZIP was not found.\n";
        exit;
    }

    $libraryReal = realpath($hotpocketContractLibraryDir);
    $fileReal = realpath($hotpocketContractLibraryDir . '/' . $name);

    // realpath containment also prevents symlinks/path traversal escaping
    // the approved smart-contract library.
    if (
        $libraryReal === false
        || $fileReal === false
        || !str_starts_with($fileReal, $libraryReal . DIRECTORY_SEPARATOR)
        || !is_file($fileReal)
        || !is_readable($fileReal)
    ) {
        http_response_code(404);
        header('Content-Type: text/plain; charset=utf-8');
        echo "Smart contract ZIP was not found or is not readable.\n";
        exit;
    }
    ...
    if ($size === false || $size < 1) {
        ...
        header('Content-Type: text/plain; charset=utf-8');
        echo "Could not read smart contract ZIP size.\n";
        exit;
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && in_array((string) ($_POST['action'] ?? ''), ['wordpress_migration_upload_chunk', 'wordpress_migration_analyze'], true)) {
    header('Content-Type: application/json; charset=utf-8');
    header('Cache-Control: no-store, max-age=0');

    $migrationAction = (string) ($_POST['action'] ?? '');
    $csrf = (string) ($_POST['csrf'] ?? '');
    $token = (string) ($_POST['token'] ?? '');

    if ($csrf === '' || !hash_equals($wpMigrationCsrf, $csrf) || $token === '' || !hash_equals($wpMigrationToken, $token)) {
        http_response_code(403);
        echo json_encode(['ok' => false, 'message' => 'The WordPress migration session expired. Reload the installer and try again.']);
        exit;
    }

    if (!$keyReadyForInstaller || !$termsAccepted || !$databaseConfigured || !$lswsPasswordConfigured || !$realIpReady || !$domainOnlyReady || $primaryApp !== '') {
        http_response_code(409);
        echo json_encode(['ok' => false, 'message' => 'WordPress migration prerequisites are not currently satisfied.']);
        exit;
    }

    $stageDir = $wpMigrationStageBase . '/' . $wpMigrationToken;

    if ($migrationAction === 'wordpress_migration_upload_chunk') {
        $kind = (string) ($_POST['kind'] ?? '');
        $originalName = basename((string) ($_POST['name'] ?? ''));
        $offsetRaw = (string) ($_POST['offset'] ?? '');
        $totalRaw = (string) ($_POST['total'] ?? '');
        $final = (string) ($_POST['final'] ?? '') === '1';
        $upload = $_FILES['chunk'] ?? null;

        if (!in_array($kind, ['files', 'sql'], true)) {
            http_response_code(400);
            echo json_encode(['ok' => false, 'message' => 'Invalid migration upload type.']);
            exit;
        }
        if ($kind === 'files' && preg_match('/\.zip$/i', $originalName) !== 1) {
            http_response_code(400);
            echo json_encode(['ok' => false, 'message' => 'WordPress files must be uploaded as a .zip archive.']);
            exit;
        }
        if ($kind === 'sql' && preg_match('/\.sql(?:\.gz|\.zip)?$/i', $originalName) !== 1) {
            http_response_code(400);
            echo json_encode(['ok' => false, 'message' => 'Database backup must be .sql, .sql.gz, or .sql.zip.']);
            exit;
        }
        if (!ctype_digit($offsetRaw) || !ctype_digit($totalRaw)) {
            http_response_code(400);
            echo json_encode(['ok' => false, 'message' => 'Invalid migration chunk offset.']);
            exit;
        }
        $offset = (int) $offsetRaw;
        $total = (int) $totalRaw;
        $maxTotal = $kind === 'files' ? 2 * 1024 * 1024 * 1024 : 4 * 1024 * 1024 * 1024;
        if ($total < 1 || $total > $maxTotal || $offset < 0 || $offset > $total) {
            http_response_code(413);
            echo json_encode(['ok' => false, 'message' => $kind === 'files' ? 'WordPress ZIP may be at most 2 GiB.' : 'SQL dump may be at most 4 GiB.']);
            exit;
        }
        if (!is_array($upload) || (int) ($upload['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK) {
            http_response_code(400);
            echo json_encode(['ok' => false, 'message' => 'Migration upload chunk failed.']);
            exit;
        }
        $tmp = (string) ($upload['tmp_name'] ?? '');
        $chunkSize = (int) ($upload['size'] ?? 0);
        if ($chunkSize < 1 || $chunkSize > 10 * 1024 * 1024 || !is_uploaded_file($tmp)) {
            http_response_code(413);
            echo json_encode(['ok' => false, 'message' => 'Migration chunks must be no larger than 10 MiB.']);
            exit;
        }

        // Migration uploads are temporary installer data, not persistent secrets.
        // Keep them in PHP's temp area so a freshly-built image needs no special
        // /home/everlomp staging-directory ownership setup.
        if (!is_dir($wpMigrationStageBase) && !@mkdir($wpMigrationStageBase, 0770, true) && !is_dir($wpMigrationStageBase)) {
            http_response_code(500);
            echo json_encode(['ok' => false, 'message' => 'Could not create the temporary WordPress migration upload directory.']);
            exit;
        }
        if (!is_dir($stageDir) && !@mkdir($stageDir, 0770, true) && !is_dir($stageDir)) {
            http_response_code(500);
            echo json_encode(['ok' => false, 'message' => 'Could not create the temporary WordPress migration session directory.']);
            exit;
        }

        $partial = $stageDir . '/' . ($kind === 'files' ? 'wordpress-files.zip.part' : 'database-upload.part');
        if ($offset === 0) {
            @unlink($partial);
            if ($kind === 'files') {
                @unlink($stageDir . '/wordpress-files.zip');
            } else {
                @unlink($stageDir . '/database.sql');
                @unlink($stageDir . '/database.sql.gz');
                @unlink($stageDir . '/database.sql.zip');
            }
        }

        clearstatcache(true, $partial);
        $currentSize = is_file($partial) ? (int) filesize($partial) : 0;
        if ($currentSize !== $offset) {
            http_response_code(409);
            echo json_encode(['ok' => false, 'message' => 'Upload offset mismatch. Restart this file upload.']);
            exit;
        }

        $in = @fopen($tmp, 'rb');
        $out = @fopen($partial, $offset === 0 ? 'wb' : 'ab');
        if (!is_resource($in) || !is_resource($out)) {
            if (is_resource($in)) fclose($in);
            if (is_resource($out)) fclose($out);
            http_response_code(500);
            echo json_encode(['ok' => false, 'message' => 'Could not write the migration upload.']);
            exit;
        }
        ...
        if ($copied !== $chunkSize) {
            http_response_code(500);
            echo json_encode(['ok' => false, 'message' => 'Migration chunk was not written completely.']);
            exit;
        }
        ...
        if ($written > $total) {
            @unlink($partial);
            http_response_code(409);
            echo json_encode(['ok' => false, 'message' => 'Migration upload exceeded the declared file size.']);
            exit;
        }
        ...
        if ($final) {
            if ($written !== $total) {
                http_response_code(409);
                echo json_encode(['ok' => false, 'message' => 'Final migration chunk size does not match the selected file.']);
                exit;
            }
            if ($kind === 'files') {
                $finalPath = $stageDir . '/wordpress-files.zip';
            } else {
                if (preg_match('/\.sql\.zip$/i', $originalName) === 1) {
                    $finalPath = $stageDir . '/database.sql.zip';
                } elseif (preg_match('/\.sql\.gz$/i', $originalName) === 1) {
                    $finalPath = $stageDir . '/database.sql.gz';
                } else {
                    $finalPath = $stageDir . '/database.sql';
                }
            }
            if (!@rename($partial, $finalPath)) {
                http_response_code(500);
                echo json_encode(['ok' => false, 'message' => 'Could not finalize the migration upload.']);
                exit;
            }
            @chmod($finalPath, 0660);
        }

        echo json_encode(['ok' => true, 'bytes' => $written, 'total' => $total, 'done' => $final]);
        exit;
    }

    // Analysis only reads the temporary migration uploads. It deliberately
    // runs as the PHP/OpenLiteSpeed worker and does not require root.
    [$code, $stdout, $stderr] = runDirectHelper(
        '/usr/local/sbin/everlomp-wordpress-migrate',
        'analyze',
        json_encode(['token' => $wpMigrationToken], JSON_UNESCAPED_SLASHES | JSON_THROW_ON_ERROR)
    );
    if ($code !== 0) {
        http_response_code(409);
        echo json_encode(['ok' => false, 'message' => $stderr !== '' ? $stderr : ($stdout !== '' ? $stdout : 'Could not analyze the WordPress migration files.')]);
        exit;
    }
    $decoded = json_decode($stdout, true);
    if (!is_array($decoded)) {
        http_response_code(500);
        echo json_encode(['ok' => false, 'message' => 'Migration analyzer returned invalid data.']);
        exit;
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete_everlomp_installfile') {
        if ($primaryApp === '') {
            ...
            if ($error === '' && is_executable('/usr/local/sbin/everlomp-wordpress-migrate')) {
                [$migrationCleanupCode, $migrationCleanupStdout, $migrationCleanupStderr] = runEverlompHelper(
                    '/usr/local/sbin/everlomp-wordpress-migrate',
                    'uninstall'
                );
                if ($migrationCleanupCode !== 0) {
                    $error = $migrationCleanupStderr !== ''
                        ? $migrationCleanupStderr
                        : ($migrationCleanupStdout !== '' ? $migrationCleanupStdout : 'Could not remove the WordPress migration helper and its passwordless sudo permission.');
                }
            }
    ...
    if ($action === 'migrate_wordpress') {
        $siteUrl = rtrim(trim((string) ($_POST['migration_site_url'] ?? '')), '/');
        $dbName = trim((string) ($_POST['migration_db_name'] ?? ''));
        $dbUser = trim((string) ($_POST['migration_db_user'] ?? ''));
        $dbPass = (string) ($_POST['migration_db_password'] ?? '');
        $dbPassConfirm = (string) ($_POST['migration_db_password_confirm'] ?? '');
        $sourceDatabase = (string) ($_POST['migration_source_database'] ?? '');
        $sourcePrefix = (string) ($_POST['migration_source_prefix'] ?? '');
        $sourceRoot = (string) ($_POST['migration_source_root'] ?? '');
        $replaceUrls = (string) ($_POST['migration_replace_urls'] ?? '') === '1';
        $csrf = (string) ($_POST['migration_csrf'] ?? '');
        $token = (string) ($_POST['migration_token'] ?? '');

        if ($csrf === '' || !hash_equals($wpMigrationCsrf, $csrf) || $token === '' || !hash_equals($wpMigrationToken, $token)) {
            $error = 'The WordPress migration session expired. Reload the page and upload/analyze the migration files again.';
        } elseif ((string) ($_POST['accept_wordpress_terms'] ?? '') !== '1') {
            $error = 'Accept the WordPress terms before migrating WordPress.';
        } elseif (!$databaseConfigured || !$lswsPasswordConfigured || !$realIpReady || !$domainOnlyReady) {
            $error = 'Complete the MariaDB, OpenLiteSpeed, Real IP and HTTPS-domain prerequisites before migrating WordPress.';
        } elseif ($primaryApp !== '') {
            $error = 'A primary application is already installed.';
        } elseif (strlen($dbPass) < 12) {
            $error = 'Use a destination database password with at least 12 characters.';
        } elseif ($dbPass !== $dbPassConfirm) {
            $error = 'The destination database passwords do not match.';
        } elseif ($sourceDatabase === '' || $sourcePrefix === '') {
            $error = 'Analyze the migration files and choose the detected WordPress database before starting migration.';
        } else {
            ignore_user_abort(true);
            @set_time_limit(0);
            $payload = [
                'token' => $wpMigrationToken,
                'site_url' => $siteUrl,
            ...
            $error = $stderr !== '' ? $stderr : ($stdout !== '' ? $stdout : 'WordPress migration failed. Uploaded migration files were kept so you can correct the issue and retry.');
        }
    }
...
<a target="_blank" rel="noopener noreferrer" class="back" href="<?= h($panelUrl) ?>">← Back to Everlomp</a>
<section class="hero"><h2>WordPress.</h2><p>Install a new site or migrate an existing WordPress ZIP and SQL backup into a fresh Everlomp database.</p></section>
<?php if ($error !== ''): ?><div class="notice error"><?= h($error) ?></div><?php endif; ?>

<div id="wordpress-installer-fragment">
<section class="card full">
<h3>What do you want to do?</h3>
<div class="db-choice">
<label class="db-option">
<input type="radio" name="wordpress_setup_mode" value="fresh" <?= ((string) ($_POST['action'] ?? '')) !== 'migrate_wordpress' ? 'checked' : '' ?> onchange="toggleWordPressSetupMode('fresh')">
<span><strong>Install fresh WordPress</strong><span>Create a new WordPress site with Everlomp's normal installer.</span></span>
</label>
<label class="db-option">
<input type="radio" name="wordpress_setup_mode" value="migrate" <?= ((string) ($_POST['action'] ?? '')) === 'migrate_wordpress' ? 'checked' : '' ?> onchange="toggleWordPressSetupMode('migrate')">
<span><strong>I want to migrate a WordPress</strong><span>Upload the existing site files and SQL dump, detect WordPress automatically, then import it.</span></span>
</label>
</div>
</section>

<div id="wordpress-fresh-panel" class="<?= ((string) ($_POST['action'] ?? '')) === 'migrate_wordpress' ? 'hidden' : '' ?>">
<section class="card full">
...
<span><strong>Install WordPress core with local file only</strong><span>Skip WordPress.org for the WordPress core archive. Themes and plugins use their own per-item source choices below. LiteSpeed Cache is skipped because no local fallback is bundled.</span></span>
</label>
...
<span><strong><?= h((string) $theme['name']) ?></strong><span><?= h((string) ($theme['description'] ?? '')) ?></span></span>
</label>
<div class="addon-actions">
<?php if (($theme['local_available'] ?? false) === true): ?><label class="inline-check"><input type="checkbox" name="wp_theme_install_local[]" value="<?= h($themeSlug) ?>" <?= in_array($themeSlug, $wpInstallLocalThemeSlugs, true) ? 'checked' : '' ?>> Install locally</label><?php endif; ?>
...
<?php else: ?><div class="warning">No valid WordPress theme add-ons were found in <code>/home/everlomp/wpaddons/themes.json</code>.</div><?php endif; ?>

<h3>Optional plugins</h3>
<p>Choose each plugin to install, whether to use its bundled ZIP immediately, and whether to activate it.</p>
<?php if ($wpPlugins !== []): ?>
<div class="db-choice">
...
<span><strong><?= h((string) $plugin['name']) ?></strong><span><?= h((string) ($plugin['description'] ?? '')) ?></span></span>
</label>
<div class="addon-actions">
<?php if (($plugin['local_available'] ?? false) === true): ?><label class="inline-check"><input type="checkbox" name="wp_plugin_install_local[]" value="<?= h($pluginSlug) ?>" <?= in_array($pluginSlug, $wpInstallLocalPluginSlugs, true) ? 'checked' : '' ?>> Install locally</label><?php endif; ?>
...
<?php else: ?><div class="warning">No valid WordPress plugin add-ons were found in <code>/home/everlomp/wpaddons/plugins.json</code>.</div><?php endif; ?>

<div class="warning"><strong>Email delivery:</strong> this Everlomp image does not include a local mail-transfer service. Configure an external SMTP/mail provider after installation.</div>

<h3>Database</h3>
<div class="db-choice">
<label class="db-option"><input type="radio" name="db_mode" value="auto" checked onchange="toggleDb('wp')"><span><strong>Create one automatically</strong><span>Everlomp creates a local database, restricted DB user, and random password.</span></span></label>
<label class="db-option"><input type="radio" name="db_mode" value="existing" onchange="toggleDb('wp')"><span><strong>Use an existing database</strong><span>Use any existing MySQL/MariaDB host, database, username, and password.</span></span></label>
</div>
<div id="wp-auto-db"><label>New database name<input type="text" name="auto_db_name" value="wordpress" maxlength="64" pattern="[A-Za-z0-9_]+"></label></div>
<div id="wp-existing-db" class="hidden"><div class="split"><label>Database host<input type="text" name="existing_db_host" value="localhost"></label><label>Database port <small>(optional)</small><input type="text" name="existing_db_port" placeholder="3306"></label></div><div class="split"><label>Database name<input type="text" name="existing_db_name"></label><label>Database username<input type="text" name="existing_db_user"></label></div><label>Database password<input type="password" name="existing_db_password" autocomplete="new-password"></label></div>

<div class="terms-agreements">
<label class="terms-check"><input type="checkbox" name="accept_wordpress_terms" value="1" required><span>I have read and agreed to the <a target="_blank" rel="noopener noreferrer" href="https://wordpress.com/tos/">terms and conditions of WordPress</a>.</span></label>
<label class="terms-check"><input type="checkbox" name="accept_everlomp_terms" value="1" required><span>I have read and agreed to the <a target="_blank" rel="noopener noreferrer" href="#everlomp-installation-terms">terms and conditions for this app, visible on this installation page</a>.</span></label>
</div>
<div class="actions"><button type="submit">Install WordPress</button><a target="_blank" rel="noopener noreferrer" class="button secondary" href="<?= h($panelUrl) ?>">Cancel</a></div>
</form>
</section>
</div>

<div id="wordpress-migration-panel" class="<?= ((string) ($_POST['action'] ?? '')) === 'migrate_wordpress' ? '' : 'hidden' ?>">
<section class="card full">
<h3>Migrate an existing WordPress</h3>
<p>The browser uploader sends <strong>8 MiB chunks</strong>, so a large backup does not require a 1 GiB PHP POST request. WordPress ZIP uploads are limited to 2 GiB compressed; SQL, SQL.GZ, or SQL.ZIP uploads are limited to 4 GiB.</p>
<div class="warning" style="margin-bottom:12px;"><strong>Safety:</strong> migration creates/uses only the destination database name you enter below. If that database already contains tables, Everlomp refuses to overwrite it. Multi-database SQL dumps are analyzed and only the WordPress database you confirm is imported.</div>

<div class="split">
<label>New site URL<input id="migration-site-url" type="url" value="<?= h((string) ($_POST['migration_site_url'] ?? $detectedSiteUrl)) ?>" required></label>
<label>New database name<input id="migration-db-name" type="text" value="<?= h((string) ($_POST['migration_db_name'] ?? 'wordpress')) ?>" maxlength="64" pattern="[A-Za-z0-9_]+" required></label>
</div>
<div class="split">
<label>New database user<input id="migration-db-user" type="text" value="<?= h((string) ($_POST['migration_db_user'] ?? 'wordpress')) ?>" maxlength="32" pattern="[A-Za-z][A-Za-z0-9_]{2,31}" required><small>Use a dedicated user; migration may update its password.</small></label>
<label>New database password<input id="migration-db-password" type="password" minlength="12" autocomplete="new-password" required></label>
</div>
<div class="split">
<label>Confirm database password<input id="migration-db-password-confirm" type="password" minlength="12" autocomplete="new-password" required></label>
<div style="display:flex;align-items:end"><button type="button" class="secondary" onclick="generateMigrationDbPassword()">Generate strong password</button></div>
</div>

<div class="wizard-divider"></div>
<h3>1. Upload WordPress files</h3>
<label>WordPress site ZIP<input id="migration-files-input" type="file" accept=".zip,application/zip" onchange="uploadWordPressMigrationFile(this,'files')"><small>ZIP may contain the WordPress root directly or inside folders such as <code>public_html/</code>.</small></label>
<div id="migration-files-progress" class="status">No ZIP uploaded yet.</div>

<h3>2. Upload database</h3>
<label>Database dump<input id="migration-sql-input" type="file" accept=".sql,.sql.gz,.sql.zip,application/sql,application/gzip,application/zip" onchange="uploadWordPressMigrationFile(this,'sql')"><small>Accepts <code>.sql</code>, <code>.sql.gz</code>, and <code>.sql.zip</code>. A SQL ZIP must contain exactly one <code>.sql</code> or <code>.sql.gz</code> dump.</small></label>
<div id="migration-sql-progress" class="status">No SQL uploaded yet.</div>

<div class="actions"><button id="migration-analyze-button" type="button" class="secondary" onclick="analyzeWordPressMigration()">Analyze migration</button></div>
<div id="migration-analysis" class="access-box hidden" style="margin-top:16px"></div>

<form method="post" autocomplete="off" id="wordpress-migration-form" onsubmit="return prepareWordPressMigrationSubmit()">
<input type="hidden" name="action" value="migrate_wordpress">
<input type="hidden" name="migration_csrf" value="<?= h($wpMigrationCsrf) ?>">
<input type="hidden" name="migration_token" value="<?= h($wpMigrationToken) ?>">
<input type="hidden" name="migration_site_url" id="migration-site-url-hidden">
<input type="hidden" name="migration_db_name" id="migration-db-name-hidden">
<input type="hidden" name="migration_db_user" id="migration-db-user-hidden">
<input type="hidden" name="migration_db_password" id="migration-db-password-hidden">
<input type="hidden" name="migration_db_password_confirm" id="migration-db-password-confirm-hidden">
<input type="hidden" name="migration_source_database" id="migration-source-database" value="<?= h((string) ($_POST['migration_source_database'] ?? '')) ?>">
<input type="hidden" name="migration_source_prefix" id="migration-source-prefix" value="<?= h((string) ($_POST['migration_source_prefix'] ?? '')) ?>">
<input type="hidden" name="migration_source_root" id="migration-source-root" value="<?= h((string) ($_POST['migration_source_root'] ?? '')) ?>">
<label class="inline-check"><input id="migration-replace-urls" type="checkbox" name="migration_replace_urls" value="1" checked> Replace the old site URL with the new URL throughout WordPress data (serialized values are handled safely; GUID columns are left unchanged).</label>
<div class="terms-agreements">
<label class="terms-check"><input type="checkbox" name="accept_wordpress_terms" value="1" required><span>I have read and agreed to the <a target="_blank" rel="noopener noreferrer" href="https://wordpress.com/tos/">terms and conditions of WordPress</a>.</span></label>
<label class="terms-check"><input type="checkbox" name="accept_everlomp_terms" value="1" required><span>I have read and agreed to the <a target="_blank" rel="noopener noreferrer" href="#everlomp-installation-terms">terms and conditions for this app, visible on this installation page</a>.</span></label>
</div>
<div class="actions"><button id="migration-start-button" type="submit" disabled>Start WordPress Migration</button><a class="button secondary" href="<?= h($panelUrl) ?>">Cancel</a></div>
</form>
</section>
</div>
</div>
...
<div class="status good">● HotPocket enabled</div>
<h3>Smart-contract runtime is active</h3>
<p>Supervisor is configured to autostart HotPocket and keep it running. Future smart-contract uploads/deployments can be done the normal way with <code>evdevkit</code>.</p>

<div class="wizard-divider"></div>
<h3>Smart Contract Library</h3>
<p>Download any saved smart-contract ZIP before deploying or sharing it so you can inspect its source code and <code>contract.deploy.json</code>.</p>

<?php if ($hotpocketContractLibrary !== []): ?>
<label>Saved smart contract
<select id="hotpocket-contract-inspect-select">
<option value="">Choose a ZIP…</option>
<?php foreach ($hotpocketContractLibrary as $libraryFile): ?>
<option value="<?= h((string) $libraryFile['name']) ?>"><?= h((string) $libraryFile['name']) ?> · <?= h(number_format(((int) $libraryFile['size']) / 1048576, 2)) ?> MiB</option>
<?php endforeach; ?>
</select>
<small>Files are served only from <code><?= h($hotpocketContractLibraryDir) ?></code>. The selected ZIP is downloaded unchanged for local inspection.</small>
</label>
<div class="wizard-actions" style="margin-top:10px">
<button type="button" class="secondary" onclick="downloadHotPocketContractFromSelect('hotpocket-contract-inspect-select')">↓ Download ZIP for inspection</button>
</div>
<?php else: ?>
<div class="warning"><strong>No saved ZIPs found.</strong> Put one or more <code>.zip</code> files in <code><?= h($hotpocketContractLibraryDir) ?></code>, then reload this step.</div>
<?php endif; ?>
...
<small>Select a contract to deploy it, or download the original ZIP first to inspect all source files and <code>contract.deploy.json</code>. Saved ZIPs live in <code><?= h($hotpocketContractLibraryDir) ?></code>.</small>
</label>
<?php if ($hotpocketContractLibrary !== []): ?>
<div class="wizard-actions" style="margin-top:10px">
<button type="button" class="secondary" onclick="downloadSelectedHotPocketContract(this.form)">↓ Download selected ZIP</button>
</div>
<?php else: ?>
<div class="warning"><strong>No saved ZIPs found yet.</strong> Put one or more <code>.zip</code> files in <code><?= h($hotpocketContractLibraryDir) ?></code>, then reload this step.</div>
<?php endif; ?>
</div>
...
const wpMigrationCsrf = <?= json_encode($wpMigrationCsrf, JSON_UNESCAPED_SLASHES) ?>;
const wpMigrationToken = <?= json_encode($wpMigrationToken, JSON_UNESCAPED_SLASHES) ?>;
let wpMigrationAnalysisReady = false;

function toggleWordPressSetupMode(mode) {
    const fresh = document.getElementById('wordpress-fresh-panel');
    const migrate = document.getElementById('wordpress-migration-panel');
    if (fresh) fresh.classList.toggle('hidden', mode !== 'fresh');
    if (migrate) migrate.classList.toggle('hidden', mode !== 'migrate');
}
...
async function uploadWordPressMigrationFile(input, kind) {
    const file = input && input.files ? input.files[0] : null;
    const status = document.getElementById(kind === 'files' ? 'migration-files-progress' : 'migration-sql-progress');
    wpMigrationAnalysisReady = false;
    const start = document.getElementById('migration-start-button');
    if (start) start.disabled = true;
    if (!file) { if (status) status.textContent = 'No file selected.'; return; }
    const lower = file.name.toLowerCase();
    if ((kind === 'files' && !lower.endsWith('.zip')) || (kind === 'sql' && !(lower.endsWith('.sql') || lower.endsWith('.sql.gz') || lower.endsWith('.sql.zip')))) {
        if (status) status.textContent = kind === 'files' ? 'Choose a .zip file.' : 'Choose a .sql, .sql.gz, or .sql.zip file.';
        return;
    }
    ...
    try {
        for (let offset = 0; offset < file.size; offset += chunkSize) {
            ...
            form.append('action', 'wordpress_migration_upload_chunk');
            form.append('csrf', wpMigrationCsrf);
            form.append('token', wpMigrationToken);
            form.append('kind', kind);
            form.append('name', file.name);
            form.append('offset', String(offset));
            form.append('total', String(file.size));
            form.append('final', end === file.size ? '1' : '0');
            form.append('chunk', file.slice(offset, end), file.name + '.part');
            const response = await fetch(window.location.href, { method: 'POST', body: form, credentials: 'same-origin' });
            const data = await response.json().catch(() => ({ok:false,message:'Invalid upload response.'}));
            if (!response.ok || !data.ok) throw new Error(data.message || 'Upload failed.');
            const pct = Math.min(100, Math.round((end / file.size) * 100));
            if (status) status.textContent = `Uploading ${pct}% · ${formatMigrationBytes(end)} / ${formatMigrationBytes(file.size)}`;
        }
        if (status) status.textContent = `✓ Uploaded ${file.name} · ${formatMigrationBytes(file.size)}`;
    } catch (error) {
...
async function analyzeWordPressMigration() {
    const box = document.getElementById('migration-analysis');
    const button = document.getElementById('migration-analyze-button');
    const start = document.getElementById('migration-start-button');
    if (!box) return;
    box.classList.remove('hidden');
    box.textContent = 'Analyzing WordPress ZIP and SQL dump…';
    if (button) button.disabled = true;
    if (start) start.disabled = true;
    wpMigrationAnalysisReady = false;
    try {
        const body = new URLSearchParams({action:'wordpress_migration_analyze',csrf:wpMigrationCsrf,token:wpMigrationToken});
        const response = await fetch(window.location.href, {method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded;charset=UTF-8'},body,credentials:'same-origin'});
        const data = await response.json().catch(() => ({ok:false,message:'Invalid analyzer response.'}));
        if (!response.ok || !data.ok) throw new Error(data.message || 'Analysis failed.');
        renderWordPressMigrationAnalysis(data);
    } catch (error) {
        box.textContent = 'Analysis failed: ' + (error instanceof Error ? error.message : String(error));
    } finally {
        if (button) button.disabled = false;
    }
}

function renderWordPressMigrationAnalysis(data) {
    const box = document.getElementById('migration-analysis');
    if (!box) return;
    box.replaceChildren();
    const title = document.createElement('div'); title.className = 'access-box-title';
    const strong = document.createElement('strong'); strong.textContent = 'Migration analysis'; title.appendChild(strong); box.appendChild(title);

    const zipP = document.createElement('p');
    zipP.textContent = `ZIP: ${data.zip.roots.length} WordPress root candidate(s), ${data.zip.entries} entries, ${formatMigrationBytes(data.zip.expanded_size)} expanded.`;
    box.appendChild(zipP);
    const rootsTitle = document.createElement('strong'); rootsTitle.textContent = 'WordPress files root'; box.appendChild(rootsTitle);
    const roots = document.createElement('div'); roots.className = 'db-choice';
    (data.zip.roots || []).forEach((root, index) => {
        const label = document.createElement('label'); label.className = 'db-option';
        const radio = document.createElement('input'); radio.type='radio'; radio.name='migration_detected_root'; radio.value=String(root.root || ''); radio.checked=index===0;
        radio.addEventListener('change', () => { if (radio.checked) document.getElementById('migration-source-root').value = radio.value; });
        const span=document.createElement('span'); const b=document.createElement('strong'); b.textContent=root.display_root || '/'; span.appendChild(b);
        const small=document.createElement('span'); small.textContent=(root.wp_config_found ? 'wp-config.php found' : 'wp-config.php not found') + (root.db_name_hint ? ` · DB hint: ${root.db_name_hint}` : '') + (root.prefix_hint ? ` · prefix hint: ${root.prefix_hint}` : ''); span.appendChild(small);
        label.append(radio,span); roots.appendChild(label);
    }); box.appendChild(roots);

    const dbTitle=document.createElement('strong'); dbTitle.textContent='Detected WordPress database'; box.appendChild(dbTitle);
    const dbs=document.createElement('div'); dbs.className='db-choice';
    (data.sql.candidates || []).forEach((cand,index) => {
        const label=document.createElement('label'); label.className='db-option';
        const radio=document.createElement('input'); radio.type='radio'; radio.name='migration_detected_db'; radio.value=String(index); radio.checked=index===0;
        const select=()=>{ if (!radio.checked) return; document.getElementById('migration-source-database').value=String(cand.database); document.getElementById('migration-source-prefix').value=String(cand.prefix); };
        radio.addEventListener('change',select);
        const span=document.createElement('span'); const b=document.createElement('strong'); b.textContent=`${cand.display_database} · ${cand.prefix}`; span.appendChild(b);
        const small=document.createElement('span'); small.textContent=`${cand.confidence} confidence · ${cand.score} WordPress core table signatures · ${cand.table_count} tables${cand.wp_config_hint_match ? ' · matches wp-config.php hint' : ''}`; span.appendChild(small);
        label.append(radio,span); dbs.appendChild(label);
    }); box.appendChild(dbs);

    const firstRoot=(data.zip.roots || [])[0]; if (firstRoot) document.getElementById('migration-source-root').value=String(firstRoot.root || '');
    const firstDb=(data.sql.candidates || [])[0]; if (firstDb) { document.getElementById('migration-source-database').value=String(firstDb.database); document.getElementById('migration-source-prefix').value=String(firstDb.prefix); }
    wpMigrationAnalysisReady = Boolean(firstRoot && firstDb);
    const start=document.getElementById('migration-start-button'); if (start) start.disabled=!wpMigrationAnalysisReady;
}

function generateMigrationDbPassword() {
    const chars='ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz23456789!@#$%^&*_-+=';
    const bytes=new Uint32Array(28); crypto.getRandomValues(bytes);
    let password=''; for (const value of bytes) password += chars[value % chars.length];
    const p=document.getElementById('migration-db-password'); const c=document.getElementById('migration-db-password-confirm');
    if (p) p.value=password; if (c) c.value=password;
}

function prepareWordPressMigrationSubmit() {
    if (!wpMigrationAnalysisReady && !document.getElementById('migration-source-database').value) {
        alert('Analyze the migration files first.'); return false;
    }
    const map = [
        ['migration-site-url','migration-site-url-hidden'],['migration-db-name','migration-db-name-hidden'],['migration-db-user','migration-db-user-hidden'],
        ['migration-db-password','migration-db-password-hidden'],['migration-db-password-confirm','migration-db-password-confirm-hidden']
    ];
    for (const [source,target] of map) {
        const s=document.getElementById(source), t=document.getElementById(target); if (s && t) t.value=s.value;
    }
    const password=document.getElementById('migration-db-password').value;
    const confirmPassword=document.getElementById('migration-db-password-confirm').value;
    if (password.length < 12) { alert('Database password must be at least 12 characters.'); return false; }
    if (password !== confirmPassword) { alert('Database passwords do not match.'); return false; }
    return window.confirm('Start the WordPress migration now? The selected destination database must be empty.');
}
...
async function openAppInstaller(app) {
    ...
    try {
        ...
        const html = await response.text();
        const doc = new DOMParser().parseFromString(html, 'text/html');
        const wordpressFragment = app === 'wordpress'
            ? doc.querySelector('#wordpress-installer-fragment')
            : null;
        const card = Array.from(doc.querySelectorAll('section.card.full')).find((node) => !node.classList.contains('terms-card'));
        const fragment = wordpressFragment || card;
        if (!fragment) throw new Error('Installer is not available. Check prerequisite steps.');
        const packageTerms = app.startsWith('external:')
            ? doc.querySelector('section.card.full.terms-card')
            : null;
        drawer.innerHTML = fragment.outerHTML + (packageTerms ? packageTerms.outerHTML : '');
        ...
        if (app === 'wordpress') {
            const selectedMode = drawer.querySelector('input[name="wordpress_setup_mode"]:checked');
            toggleWordPressSetupMode(selectedMode ? selectedMode.value : 'fresh');
        }
...
async function submitAppInstaller(event) {
    ...
    try {
        ...
        if (fragment) {
            const drawer = document.getElementById('app-installer-drawer');
            drawer.innerHTML = fragment.outerHTML;
            drawer.querySelectorAll('form').forEach((newForm) => {
...
function startHotPocketContractDownload(name) {
    ...
    url.search = '';
    url.hash = '';
    url.searchParams.set('download', 'hotpocket-contract');
...
function downloadSelectedHotPocketContract(form) {
    if (!(form instanceof HTMLFormElement)) return;

    const select = form.querySelector('select[name="hotpocket_contract_library_file"]');
    const name = select?.value || '';

    if (!name) {
        alert('Choose a saved smart contract ZIP first.');
        select?.focus();
        return;
    }

    startHotPocketContractDownload(name);
}

function downloadHotPocketContractFromSelect(id) {
    const select = document.getElementById(id);
    const name = select?.value || '';

    if (!name) {
        alert('Choose a smart contract ZIP first.');
        select?.focus();
        return;
    }
